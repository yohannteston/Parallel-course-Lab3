...
int main(int argc, char *argv[]) {
	...
	omp_set_nested(1); //only to use nested loops

	//the following pragma is used alone to parallelize only the j-loop
//#pragma omp parallel for private(rank,i) 
	for (j=0;j<len;j++)
	{

		rank = 0;
		//the following pragma is used alone to parallelize only the i-loop
